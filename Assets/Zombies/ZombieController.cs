    ...
    void Update()
    {
        ...
        switch (state)
        {
            ...
            case STATE.CHASE:
                ...
                if(agent.remainingDistance <= agent.stoppingDistance && !agent.pathPending) //pathPending ensures that we have a path to the player (we might not get it straight away)
                {
                    state = STATE.ATTACK;
                }
